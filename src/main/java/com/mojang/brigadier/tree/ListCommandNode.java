...
public class ListCommandNode<S, T> extends ArgumentCommandNode<S, T> {
  ...
  public void parse(StringReader reader, CommandContextBuilder<S> contextBuilder)
    ...
    ParsedArgument<S, List<T>> parsed =
        (ParsedArgument<S, List<T>>) contextBuilder.getArguments().get(getName());
    if (parsed == null) {
      parsed = new ParsedArgument<>(start, reader.getCursor(), new ArrayList<>());
    } else {
      parsed =
          new ParsedArgument<>(
              parsed.getRange().getStart(), reader.getCursor(), parsed.getResult());
    }
